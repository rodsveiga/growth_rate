...
parser.add_argument('--map', dest= 'plot_map', default= False, type= bool,
                    help= 'Plot maps') 
...
for locs in locs_:
    ...
    if args.save_figdata:
        ...
        if args.state_or_city == 'city':
            df_.to_csv(path_output_ + '/df_figures_%s.csv' % str(cities), index= True, sep= ';')
        else:
            df_.to_csv(path_output_ + '/df_figures_%s.csv' % str(locs), index= True, sep= ';')
...
if args.plot_map:
    ...
    if args.state_or_city == 'city':
        ...
        shapefile="data/BR_Municipios_2019.shp"
        geodiv=geopandas.read_file(shapefile)
        #geodiv.rename(columns={'CD_GEOCMU':'city_ibge_code'}, inplace=True)
        #print(geodiv.columns.values)
        geodiv = geodiv.rename(columns={'CD_MUN':'city_ibge_code'})
       
    elif args.state_or_city == 'state':
            #shapefile="data/BRUFE250GC_SIR.shp"
            shapefile="data/BR_UF_2019.shp"
            geodiv=geopandas.read_file(shapefile)
            #geodiv.rename(columns={'CD_GEOCUF':'city_ibge_code'}, inplace=True)
            #print(geodiv.columns.values)
            geodiv = geodiv.rename(columns={'CD_UF':'city_ibge_code'})
            #geodiv = geodiv.rename(columns={'CD_GEOCUF':'city_ibge_code'})
    ...
    df_map.plot(column='CLASSIFICACAO', categorical=True, cmap='magma', linewidth=.5, edgecolor='0.6',
         legend=True, legend_kwds={'bbox_to_anchor':(.35, 0.4),'fontsize':20,'frameon':False}, ax=ax)
    ax.axis('off')
    plt.tight_layout()
    plt.savefig(path_output_ + '/' + 'map_brazil'  + '.png',dpi=300, bbox_inches='tight')
    plt.close(fig)
    ...
    if args.state_or_city == 'city':
        for ST in results.state.unique():
            ...
            fig, ax = plt.subplots(1, figsize=(28,16))
            df_ST.plot(column='CLASSIFICACAO', categorical=True, cmap='magma', linewidth=.5, edgecolor='0.6',
             legend=True, legend_kwds={'bbox_to_anchor':(.9, 0.1),'fontsize':20,'frameon':False}, ax=ax)
            ax.axis('off')
            plt.tight_layout()
            plt.savefig(path_output_ + '/' + 'map_' + ST + '.png', dpi=300, bbox_inches='tight')
